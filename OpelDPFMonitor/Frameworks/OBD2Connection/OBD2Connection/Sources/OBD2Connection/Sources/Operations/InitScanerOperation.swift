...
class InitScanerOperation: StreamHandleOperation {
    ...
    private func continueInitialization() {
        ...
        reader = StreamReader(stream: input)
    ...
    private func onReadEnd() {
        ...
        case .echoOff:
            guard let resp = reader.response, resp.elmOK else {
                ...
                error = InitializationError.EchoOffError
                return
            }
        case .protocol:
            guard let response = reader.response else {
                print("Handle protocol setup error")
                error = InitializationError.ProtocolError
                return
            }
...
private func buildSupportedSensorList(data: Data, pidGroup: Int) -> Bool {
    ...
    var supportedSensorList = Array(repeating: 0, count: 16)
